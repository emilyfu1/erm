...
experimental = read_dta('https://users.nber.org/~rdehejia/data/nsw.dta')
...
logit_effects = function(fit, predictor) {
  ...
  fakeperson = predict(fit, newdata=mean_data)[[1]]
  p_avg = dlogis(fakeperson)
  ...
  p_all = predict(fit, type='response')
  APE = mean(bhat * p_all, na.rm=TRUE)
...
The regression model and the null prediction have about the same accuracy. The model has low sensitivity, so it isn't good at predicting when someone does switch wells but is better at predicting when someone does not switch. Since the null model always predicts switch == 1, there are no false negatives or true negatives. Its sensitivity and specificity are one and zero, respectively.
...
A 100-metre increase in distance from a safe well is associated with a 32.4 percentage point decrease in the average household's probability of switching wells. A 1-unit increase in level of arsenic is associated with a 29.4 percentage point increase in the average household's probability of switching wells. A 1-unit increase in years of education of the head of the household is associated with a 5.7 percentage point increase in the average household's probability of switching wells.
...
This model does better on just accuracy than the null model and fit1. It has a higher sensitivity and a lower specificity than fit1, so it's relatively better at predicting a switcher (compared to fit1). 
