...
spec5a_controlmeans = patiencedataclean |> 
  filter(tr16 == 0, tr8 == 0) |> 
  summarise(mean(convex1, na.rm=TRUE)) |> 
  pull()

spec5b_controlmeans = patiencedataclean |> 
  filter(tr16 == 0, tr8 == 0) |> 
  summarise(mean(convex1, na.rm=TRUE)) |> 
  pull()

spec5c_controlmeans = patiencedataclean |> 
  filter(tr16 == 0, tr8 == 0) |> 
  summarise(mean(convex3, na.rm=TRUE)) |> 
  pull()

spec5d_controlmeans = patiencedataclean |> 
  filter(tr16 == 0, tr8 == 0) |> 
  summarise(mean(convex3, na.rm=TRUE)) |> 
  pull()
...
bhat_allt_convex1 = coef(spec5a)['allt']
bhat_allt_convex3 = coef(spec5c)['allt']

# get all the estimated coefficients from the regressions with both tr16 and tr8
bhat_tr16_convex1 = coef(spec5b)['tr16']
bhat_tr8_convex1  = coef(spec5b)['tr8']
bhat_tr16_convex3 = coef(spec5d)['tr16']
bhat_tr8_convex3  = coef(spec5d)['tr8']
...
```{r}
# specifications to put into modelsummary with columns
model_list_table6 = list('(1)' = spec5a,
                         '(2)' = spec5b,
                         '(3)' = spec5c,
                         '(4)' = spec5d)
