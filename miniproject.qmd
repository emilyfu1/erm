...
spec3_sd_effect = coef(spec3)['tr16'] / spec3_sd_control
...
spec4_sd_control = patiencedataclean |>
  ...
  pull()
# get the estimated coefficient and divide by standard deviation of the outcome
spec4_sd_effect = coef(spec4)['tr16'] / spec4_sd_control
```

Produce the table:
```{r}
# specifications to put into modelsummary with columns
model_list_table5 = list('Early Choices: Today vs. 1 Week' = spec3,
                  'Early Choices: 1 Week vs. 2 Weeks' = spec4)

# need the control means to match the table in the paper
controlmean_table5 = tibble(
  term = c('Control Mean', 'Standard Deviation Effect'),
  `Early Choices: Today vs. 1 Week` = c(sprintf('%.2f', spec3_controlmeans),
                                       sprintf('%.2f', spec3_sd_effect)),
  `Early Choices: 1 Week vs. 2 Weeks` = c(sprintf('%.2f', spec4_controlmeans),
...
modelsummary(model_list_table5,
             ...
             stars = c('*' = 0.1, '**' = 0.05, '***' = 0.01),
             gof_omit = 'R2|Adj|Std.Errors', # removes unneccesary stuff
             add_rows = controlmean_table5, # add the custom row
             output = 'markdown')
```
