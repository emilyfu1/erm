...
Here, I find that treatment effects are persistent for only those with high academic standing as initially measured, while their short-term treatment effects are nearly insignificant. For the students with low academic standing, the short-term treatment effects are significant but dissipate in middle school. This is the same result found by the authors in the corresponding part of the heterogeneity analysis.
```{r echo=FALSE}
...
Another part of the figure estimates treatment effects and persistence for girls and boys separately. This part of the replication is where my results differ from those in the paper. For girls, the treatment effect does not appear to be significant nor persistent, whereas for boys it is. This is not the case in the paper findings themselves. 
```{r echo=FALSE}
...
Finally, the authors estimate treatment effects for those initially measured with high or low family SES. For students with low SES, only long-term effects are significant, while the opposite is true for students with high SES.
```{r echo=FALSE}
# elementary school
